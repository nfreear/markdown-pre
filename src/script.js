...
function getStylesheetUrl () {
  ...
  const $_SCRIPT = DOC.querySelector(IS_CDN_TEST ? 'script[ data-cdn-src ]' : 'script[ src *= markdown-pre ]')

  const SCRIPT_URL = $_SCRIPT.getAttribute(IS_CDN_TEST ? 'data-cdn-src' : 'src' )
